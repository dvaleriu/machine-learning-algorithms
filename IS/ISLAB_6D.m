function [Mid,Did,Dva, Viid, Viva] = ISLAB_6D(F,B,C,D,nk,N,sigma,lambda) 
%
         ...
         disp([blanks(22) 'nc = ' int2str(nc-1)]) ;
          ...
          if ((nf>1) || (nb>1) || (nc>1) || (nd>1))
             if ((nb<2) && (nc<2) && (nd<2))		% Model estimation. 
                ...
                Mid.b = 0 ; 
                Mid.nk = nk ;
             elseif (nb<2) % Here we can have FIR and not standard models
                Mid = bj_e(Did, [0, nc-1, nd-1, nf-1, nk]) ; 
                Mid.b = 0 ; 
                Mid.nk = nk ;
             else
                Mid = bj_e(Did,[nb-1 nc-1 nd-1 nf-1 nk]) ; % include OE models
             end 
...
Eid = Eid(nb,nc,nd,nf) ; 
Eva = Eva(nb,nc,nd,nf) ; 
Viidx = Viid(nb,nc,nd,nf) ; 
Vivax = Viva(nb,nc,nd,nf) ; 
...
figure(FIG),clf
   ...
   subplot(325)
      ...
      axis([0 K+1 ys(1)-0.05*dy ys(2)+0.2*dy]) ;
      text(K/2,ys(2)+0.05*dy, ... 
           ['Validation index = ' num2str(Viidx)]) ; 
   subplot(326)
      ...
      axis([0 K+1 ys(1)-0.05*dy ys(2)+0.2*dy]) ;
      text(K/2,ys(2)+0.05*dy, ... 
           ['Validation index = ' num2str(Vivax)]) ; 
      set(FIG,'DefaultTextHorizontalAlignment','right') ; 
      text(1.3*K,ys(1)-0.4*dy,'<Press a key>') ;
FIG = FIG+1 ; 
pause ;
